...
public class FormatChecker {
    ...
    public boolean checkNicknameFormat(String s) {
        if (matchFail(s, ".{5,}", LoginRegError.NICK_MIN_LENGTH)) return false;
        else if (matchFail(s, "\\w+", LoginRegError.NICK_FORMAT)) return false;
        else if (matchFail(s, ".{5,20}", LoginRegError.NICK_MAX_LENGTH)) return false;
        return true;
    }
